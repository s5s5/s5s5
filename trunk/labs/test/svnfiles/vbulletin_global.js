...
{
	window.open("reputation.php?" + SESSIONURL + "p=" + postid, "Reputation", "toolbar=no, scrollbars=yes, resizable=yes, width=430, height=260");
	return false;
}
function reputation_change(reputationid)
{
	window.open("reputation.php?" + SESSIONURL + "do=change&reputationid=" + reputationid, "Reputation_Change", "toolbar=no, scrollbars=yes, resizable=yes, width=550, height=380");
	return false;
}
...
// toggle quote
// cynix
function toggle_quote(o){
 p=o;
 while(o.id!='quote_x')o=o.parentNode;
 while(o.id!='quote_d'){
  c=o.childNodes;
  for(i=0;i<c.length;i++)
   if(c.item(i).id=='quote_x'||c.item(i).id=='quote_d'){
    o=c.item(i);
    break;
   }
 }
 if(o.style.height=='auto'&&o.scrollHeight>=120){
 ...
 if(o.scrollHeight<120)p.style.display='none';
 return false;
}
function toggle_quote_onload(){
 c=document.getElementsByTagName('div');
 for(i=0;i<c.length;i++){
  o=c.item(i);
  if(o.id=='quote_d'&&o.scrollHeight<120){
   o.style.height='auto';
   o.style.overflow='visible';
  }
 }
